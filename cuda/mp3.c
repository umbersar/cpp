...
__global__ void matrixMultiplyShared(float * A, float * B, float * C,
    ...
    {
      if((y < numARows) && m*TILE_WIDTH+tx<numAColumns)
      {
      	ds_A[ty][tx] = A[y*numAColumns + m*TILE_WIDTH+tx];//A[x*numAColumns + m*TILE_WIDTH+tx];
      }
      else
        ds_A[ty][tx] = 0;
      
      if(m*TILE_WIDTH+ty<numBRows && x < numBColumns )
      {
	  	ds_B[ty][tx] = B[(m*TILE_WIDTH+ty)*numBColumns+x];
      }
      else
        ds_B[ty][tx]=0;
