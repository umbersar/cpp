...
void IterativeRoot(double(*f)(double), double initValue) {
	...
	while (true) {
		if (index == 500000) {
			printf("Could not solve %s using iterative solver in %d iterations. Relax the max iteration count to see if a solution is possible\n", function_map[f].c_str(), index);
			break;
		}
		newValue = f(lastValue);
		if (fabs(newValue - lastValue) <= .000001) {
			printf("Root of the equation %s using iterative solver found! It is: %lf\n", function_map[f].c_str(), newValue);
			break;
		}
		else {
			lastValue = newValue;
