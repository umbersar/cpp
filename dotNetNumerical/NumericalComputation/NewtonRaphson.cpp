...
void IterativeRootNewtonRaphson(double(*f)(double), double initValue) {
	...
	while (true) {
		if (index == 500000) {
			printf("Could not solve %s using Newton Raphson iterative solver in %d iterations. Relax the max iteration count to see if a solution is possible.\n", function_map[f].c_str(), index);
		...
		if (fabs(newValue - lastValue) <= .000001) {
			printf("Root of the equation %s using Newton Raphson iterative solver found! It is: %lf\n", function_map[f].c_str(), newValue);
			break;
		}
		else {
			lastValue = newValue;
